...
try:
    # Required for this tests to work
    ...
    from setenum.django import DjangoChoicesSetEnumMeta
except ImportError:
    pytestmark = [pytest.mark.skip('Django is not installed')]




class Suit(models.IntegerChoices, metaclass=DjangoChoicesSetEnumMeta):
    DIAMOND = 1, _('Diamond')
    SPADE = 2, _('Spade')
    HEART = 3, _('Heart')
    CLUB = 4, _('Club')
...
class YearInSchool(models.TextChoices, metaclass=DjangoChoicesSetEnumMeta):
...
class Vehicle(models.IntegerChoices, metaclass=DjangoChoicesSetEnumMeta):
    CAR = 1, 'Carriage'
    TRUCK = 2
    JET_SKI = 3
...
class Gender(models.TextChoices, metaclass=DjangoChoicesSetEnumMeta):
    MALE = 'M'
    FEMALE = 'F'
    NOT_SPECIFIED = 'X'
...
class ChoicesTests(SimpleTestCase):
    ...
    def test_textchoices_blank_value(self):
        class BlankStr(models.TextChoices, metaclass=DjangoChoicesSetEnumMeta):
            EMPTY = '', '(Empty)'
            ONE = 'ONE', 'One'
    ...
    def test_invalid_definition(self):
        ...
        with self.assertRaisesMessage(TypeError, msg):
            class InvalidArgumentEnum(models.IntegerChoices, metaclass=DjangoChoicesSetEnumMeta):
                # A string is not permitted as the second argument to int().
                ONE = 1, 'X', 'Invalid'

        msg = "duplicate values found in <SetEnum 'Fruit'>: PINEAPPLE -> APPLE"
        with self.assertRaisesMessage(ValueError, msg):
            class Fruit(models.IntegerChoices, metaclass=DjangoChoicesSetEnumMeta):
                APPLE = 1, 'Apple'
...
class Separator(bytes, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
...
class Constants(float, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
    PI = 3.141592653589793, 'π'
    TAU = 6.283185307179586, 'τ'


class Set(frozenset, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
    A = {1, 2}
    B = {2, 3}
    UNION = A | B
    DIFFERENCE = A - B
...
class MoonLandings(datetime.date, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
...
class DateAndTime(datetime.datetime, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
    A = 2010, 10, 10, 10, 10, 10
    B = 2011, 11, 11, 11, 11, 11
    C = 2012, 12, 12, 12, 12, 12


class MealTimes(datetime.time, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
    BREAKFAST = 7, 0
    LUNCH = 13, 0
    DINNER = 18, 30


class Frequency(datetime.timedelta, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
...
class Number(decimal.Decimal, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
    E = 2.718281828459045, 'e'
    PI = '3.141592653589793', 'π'
    TAU = decimal.Decimal('6.283185307179586'), 'τ'


class IPv4Address(ipaddress.IPv4Address, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
    LOCALHOST = '127.0.0.1', 'Localhost'
    GATEWAY = '192.168.0.1', 'Gateway'
    BROADCAST = '192.168.0.255', 'Broadcast'


class IPv6Address(ipaddress.IPv6Address, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
    LOCALHOST = '::1', 'Localhost'
    UNSPECIFIED = '::', 'Unspecified'


class IPv4Network(ipaddress.IPv4Network, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
    LOOPBACK = '127.0.0.0/8', 'Loopback'
    LINK_LOCAL = '169.254.0.0/16', 'Link-Local'
    PRIVATE_USE_A = '10.0.0.0/8', 'Private-Use (Class A)'


class IPv6Network(ipaddress.IPv6Network, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
...
class CustomChoicesTests(SimpleTestCase):
    ...
    def test_bool_unsupported(self):
        ...
        with self.assertRaisesMessage(TypeError, msg):
            class Boolean(bool, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
                pass

    def test_timezone_unsupported(self):
        msg = "type 'datetime.timezone' is not an acceptable base type"
        with self.assertRaisesMessage(TypeError, msg):
            class Timezone(datetime.timezone, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
                pass

    def test_uuid_unsupported(self):
        msg = 'UUID objects are immutable'
        with self.assertRaisesMessage(TypeError, msg):
            class Identifier(uuid.UUID, models.Choices, metaclass=DjangoChoicesSetEnumMeta):
